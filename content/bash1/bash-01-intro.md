...
For the hands-on work, we have set up a small training cluster *sssfu.c3.ca* that features the same software
...
Let's log in to *sssfu.c3.ca* using a username userXX (where XX is three digits):

```sh
[local]$ ssh userXX@sssfu.c3.ca   # password supplied by the instructor
```
