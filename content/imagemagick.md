...
- Understand 200+ image file formats
- *Command line tools* and *APIs* for various languages: PythonMagick, RMagick, ImageMagick.jl, Magick++, and
  many others
...
Binary packages for Linux/Mac/Windows at https://imagemagick.org/script/download.php . On a Mac you can use `brew`:
...
On the Alliance clusters, as of this writing, ImageMagick 7.0.10-7 is part of gentoo/2020 (loaded by default).
...
where <tool> is one of: `animate`, `compare`, `composite`, `conjure`, `convert`, `display`, `identify`,
`import`, `mogrify`, `montage`, `stream`. I'll show where to find this list in a few minutes.
...
solution somewhere on [this page](https://wgpages.netlify.app/pytables).
{{< /bigQuestion >}}
...
`mogrify` is similar to `convert`, but it overwrites the original image by default, so can be used on a batch
of images (in-place batch processing).
...
`trim` does not work very well in this two-step process, but you can get better results by combining the two
commands into one:
...
Drawing falls into more advanced ImageMagick usage as it can get complicated very quickly. Here I will show
just a few commands.
...
xc:<colour> gives window fill colour
-pointsize <size> sets the font size
...
for font in AvantGarde-Book AvantGarde-BookOblique AvantGarde-Demi AvantGarde-DemiOblique Bookman-Demi; do
...
# overlay over a checked background; dst_over means "destination is composited over the source"
composite -compose dst_over -tile pattern:checkerboard helloTransparent.png helloChecked.png   # no hidden text

# The hidden text is still present in the transparent image, even though you cannot see it. Let's turn the
# alpha channel off in the transparent image:
...
composite -compose dst_over -tile pattern:checkerboard h1.png 111.png

# helloTransparent.png has 0% < alpha < 100%; let's add 75% to it (it'll truncate values over 100%)
convert helloTransparent.png -channel alpha -evaluate add 75% h2.png   # now 75% < alpha < 100%
composite -compose dst_over -tile pattern:checkerboard h2.png 111.png
...
composite -compose dst_over o40.avif d40.avif 111.avif

magick -list evaluate   # print all -evaluate operators
```
...
```sh
convert -size 500x500 xc:black 111.png                         # empty image
mogrify -fill red -draw "rectangle 0,0 250,250 " 111.png       # add red quadrant
mogrify -fill green -draw "rectangle 251,0 500,250 " 111.png   # add green quadrant
mogrify -fill blue -draw "rectangle 0,251 250,500 " 111.png    # add blue quadrant
convert 111.png -fill turquoise -opaque black 222.png   # change "opaque" color to "fill" color

convert ocean.avif -fill turquoise -opaque "#094C9A" o1.avif   # 094C9A is deep blue; change just those pixels
convert ocean.avif -fuzz 10% -fill turquoise -opaque "#094C9A" o1.avif   # broaden your colour match
```
...
```sh
ssh -X cedar
display image.png   # for this need an X11 server on your computer (XQuartz on MacOS); click on the image for
# menu
```
...
magick -list color   # list all 678 named colours
```
